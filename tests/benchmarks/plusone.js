...
window['___jsl']['f'] = ['googleapis.client', 'plusone']; // Input 0
...
(function () {
  ...
  function normalizeKey(key) {
    var featureParts = key.split("!"),
      req = normalize(featureParts[0].split(":")),
      loaded = featureParts[1] && normalize(featureParts[1].split(":"));
  ...
  var QUERY_PATTERN = /\?[&|(\S*=\S*&)]*jsh=(\S*)#?/,
    FRAGMENT_PATTERN = /#[&|(\S*=\S*&)]*jsh=(\S*)/,
    VALID_SCS_URL_REGEX = /^https:\/\/ssl.gstatic.com\/webclient\/js(\/[a-zA-Z0-9_\-]+)*\/[a-zA-Z0-9_\-\.:!]+\.js$/,
    VALID_GOOGLE_URL_REGEX = RegExp("^(http:|https:)?(\\/\\/)?([a-zA-Z0-9_\\-]+\\.)*google\\.com(:[0-9]+)?(\\/[a-zA-Z0-9_\\-]+)*\\/[a-zA-Z0-9_\\-\\.:!]+\\.js(\\?[a-zA-Z0-9_\\-&=%]*)?$"),
    type, host, container, repository, descriptors, loadedFeatures, console, parentUrl;
  initialize(document.location.href);
  jsloader_load = function (features, opt_callback) {
    var urlToLoad;
...
// Input 0
function gapi_load(features, opt_callback) {
  jsloader_load(features.split(":"), opt_callback)
};
