...
function SNote(serverPath, divParent) {
  function init(divParent$$1, serverPath$$1) {
    ...
    JAM.set(label, "textContent", "Are you sure you want to delete?", JAM.policy.p12);
  ...
  function createPasswordPanel(parent$$1, strPrompt, strPasswordInputId) {
    ...
    JAM.set(labelPassword, "textContent", strPrompt, JAM.policy.p12);
  ...
  function handleDeleteDialogOk() {
    ...
    function v0(strId) {
      JAM.set(m_preNote, "textContent", "", JAM.policy.p12);
      fetch(m_strId, handleSuccessfulFetch, handleFailedFetch);
      return;
    }
    deleteSNote(v0, v1);
  ...
  function handleUnlockDialogOk() {
    JAM.set(m_preNote, "textContent", message, JAM.policy.p12);
    closeUnlockDialog();
...
function SNoteSwindler(serverPath$$4, divParent$$2, strNoteId) {
  function createButton(strLabel, strFunc, parent$$5) {
    ...
    button.type = "input";
    JAM.set(button, "textContent", strLabel, JAM.policy.p12);
  ...
  function forgeNote() {
    ...
    JAM.set(preNote, "textContent", "Forged note", JAM.policy.p12);
    return;
  }
  function forgeNoteInnerHTML() {
    preNote = document.getElementById("note");
    JAM.set(preNote, "innerHTML", "Forged note", JAM.policy.p5);
    return;
  }
  function readNote() {
    preNote = document.getElementById("note");
